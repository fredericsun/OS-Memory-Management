...
int
checkbound(uint addr, struct proc *p)
{
  if(addr < 0x2000 || (addr >p->sz && addr < p->stsz) || addr >USERTOP) {
    return -1;
  }
  return 0;
}
...
{
  if(p->pid != 1){
    if(checkbound(addr, p) == -1 || checkbound(addr+4, p) == -1)
      return -1;
  }
    *ip = *(int*)(addr);
    return 0;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(checkbound((uint)i, proc) == -1 || checkbound((uint)i + size, proc) == -1)
    return -1;
  *pp = (char*)i;
  return 0;
}
